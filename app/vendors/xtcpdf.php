...
class XTCPDF  extends TCPDF{
	...
	function XTCPDF($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false){
		...
		$this->SetAuthor("Cordoba Soft IT - info@cordobasoft.com - @2011 ". (date('Y') > 2011 ? " - ".date('Y') : "") ." All Right Reserved");
...
//		$this->SetAutoPageBreak($this->pageBreak, $this->bMargin * $this->bMargen);
	...
	function Header(){
		...
		$this->linea[0]['posx'] = 105;
		$this->linea[0]['ancho'] = $ancho;
		$this->linea[0]['texto'] = $this->titulo['titulo4'];
		$this->linea[0]['size']  = 8;
		$this->linea[0]['borde'] = '';
		$this->linea[0]['align'] = 'R';
		$this->linea[0]['style'] = '';
...
//			$this->Reset();
	...
	function Reset(){
		...
		$this->SetFont(PDF_FONT_NAME_MAIN,'',$this->actualFontSize); // Changeable?(not yet...)
  		$this->lineheight = 5 * $this->actualFontSize / 11;         // Related to FontSizePt == 11
    	$this->SetFillColor($colorf['R'],$colorf['G'],$colorf['B']);
    	$this->SetTextColor($colort['R'],$colort['G'],$colort['B']);
    	$this->SetDrawColor($colorb['R'],$colorb['G'],$colorb['B']);
