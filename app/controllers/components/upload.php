...
ini_set('memory_limit','64M');
	...
	class UploadComponent extends Object {
		...
		function upload ($file, $destination, $name = NULL, $rules = NULL) {
			...
			if (isset($file) && is_array($file) && !$this->upload_error($file['error'])) {
				...
				// die if not correct extension
				$allowed = array('xls','doc','pdf','gif','jpg','jpeg','png');
				if(!in_array($this->ext($fileName),$allowed)){
//					exit;
//					die("File type not allowed.");
		...
		function newname ($file) {
			return time().rand(1000,9999) . "." . $this->ext($file);
		}
