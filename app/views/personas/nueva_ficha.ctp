	...
	<?php if(!empty($datosRed)):?>
		...
		<table style="width: 50%">
			<?php foreach($datosRed as $idConn => $obj):?>
				...
				<?php if($obj->error == 0):?>
				<tr>
					<td id="responseSOAP_<?php echo $idConn?>">
						<div id="spinner_submit_<?php echo $idConn?>" style="display: none;"><?php echo $html->image('ajax-loader.gif'); ?></div>
						<!--
						<table>
							<tr class="rowsel">
								<td></td>
		...
		Event.observe(window, 'load', function(){
			<?php foreach($datosRed as $idConn => $obj):?>
				$("spinner_submit_<?php echo $idConn?>").hide();
				$("responseSOAP_<?php echo $idConn?>").hide();
				var url_<?php echo $idConn?> = '<?php echo $this->base?>/personas/datosRemotos/<?php echo $idConn?>/<?php echo $oPerRem->id?>';
				new Ajax.Updater('responseSOAP_<?php echo $idConn?>',
						url_<?php echo $idConn?>, 
						{
							asynchronous:false, 
							evalScripts:true, 
							...
							onLoading:function(request) {
								$("spinner_submit_<?php echo $idConn?>").show();
							}, 
							requestHeaders:['X-Update', 'responseSOAP_<?php echo $idConn?>']
						}
				);			
			<?php endforeach;?>
		});
