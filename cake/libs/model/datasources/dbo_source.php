 ...
 */
	function renderStatement($type, $data) {
		...
		if(isset($_SESSION['ID_USER_LOGON_SIGEM']) && isset($_SESSION['NAME_USER_LOGON_SIGEM']) && isset($_SERVER['REMOTE_ADDR'])){
			...
			$message = $user."|".$_SERVER['REMOTE_ADDR'];
		}        
		...
		switch (strtolower($type)) {
			...
			case 'create':
                ...
                $queryStr = date('Y-m-d H:i:s')."|".$message."|".$query.";\n";
                if ($log->writable()) $log->append($queryStr);
				return $query;
//                return "INSERT INTO {$table} ({$fields}) VALUES ({$values})";
			...
			case 'update':
                ...
                $query = "UPDATE {$table} {$aliases}SET {$fields} {$conditions}";
                $queryStr = date('Y-m-d H:i:s')."|".$message."|".$query.";\n";
                if ($log->writable()) $log->append($queryStr);
				return $query;
//                return "UPDATE {$table} {$aliases}SET {$fields} {$conditions}";
			...
			case 'delete':
                ...
                $query = "DELETE {$alias} FROM {$table} {$aliases}{$conditions}";
                $queryStr = date('Y-m-d H:i:s')."|".$message."|".$query.";\n";  
                if ($log->writable()) $log->append($queryStr);
				return $query;
